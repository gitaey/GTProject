package com.gtp.domain.lostark.raid.service;
...
import com.gtp.domain.lostark.raid.dto.PartyMember;
...
public class GoogleSheetsService {
    ...
    public void init() {
        try {
            java.io.InputStream credStream;
            if (credentialsJson != null && !credentialsJson.isBlank()) {
                credStream = new java.io.ByteArrayInputStream(credentialsJson.getBytes(java.nio.charset.StandardCharsets.UTF_8));
            } else if (credentialsResource != null) {
                credStream = credentialsResource.getInputStream();
            } else {
                log.warn("Google Sheets credentials 미설정 - 서비스 비활성화");
                return;
            }
