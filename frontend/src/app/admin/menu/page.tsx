...
// 슈퍼관리자 전용 그룹 (블로그, 기빵봇은 슈퍼관리자만 관리)
const SUPER_ONLY_GROUPS = new Set(['블로그', '기빵봇'])
...
function getVisibleCatalog(isSuper: boolean): CatalogArea[] {
    if (isSuper) return MENU_CATALOG
    return MENU_CATALOG.map((area) => ({
        ...area,
        groups: area.groups.filter((g) => !SUPER_ONLY_GROUPS.has(g.groupLabel)),
    }))
}

const SUPER_ONLY_MENU_IDS = new Set(
    MENU_CATALOG.flatMap((area) =>
        area.groups.filter((g) => SUPER_ONLY_GROUPS.has(g.groupLabel)).flatMap((g) => g.items.map((i) => i.id)),
    ),
)
...
async function fetchRoles(): Promise<RoleItem[]> {
    ...
    if (!json.success) throw new Error(json.message ?? '역할 목록 조회 실패')
...
export default function MenuManagementPage() {
    ...
    const roleOption = roles.find((r) => r.code === selectedRole)
    const permOptions = roleOption?.permissions?.slice().sort((a, b) => a.sortOrder - b.sortOrder) ?? []
    const effectivePermission = roleOption?.hasSubPermission
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const filtered = roleOption?.isSuper ? ids : ids.filter((id) => !SUPER_ONLY_MENU_IDS.has(id))
            setChecked(new Set(filtered))
        } catch (e) {
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="flex-1 p-6 space-y-5">
                    ...
                    >
                        ...
                        {roleOption?.hasSubPermission && permOptions.length > 0 && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                                <label
                                    style={{
                                        fontSize: '11px',
                                            ...
                                            {area.groups.map((group, gi) => (
                                                <div key={group.groupLabel}>
                                                    ...
                                                    >
                                                        ...
                                                        <div
                                                            style={{ flex: 1, height: '1px', background: '#f0f2f5' }}
                                                        />
                                                    </div>
                                                            ...
                                                            <label
                                                                ...
                                                                onMouseLeave={(e) => {
                                                                    ;(e.currentTarget as HTMLElement).style.background =
                                                                        on ? 'rgba(242,103,34,0.04)' : 'transparent'
                                                                }}
                                                            >
                                                                <input
                                                                    type="checkbox"
